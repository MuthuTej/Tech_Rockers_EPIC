    ...
    <div className="space-y-5">
            ...
            <div key={b.id} className="liq-card p-4" style={{ borderLeft: '4px solid #EAB308' }}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] font-mono" style={{ color: '#3b82f6' }}>{b.id}</div>
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  <button onClick={() => setConfirm({ kind: 'approve', block: b })}
                    className="liq-btn text-xs px-4" style={{ background: '#86efac', color: '#0B0F1A' }}>
            ...
            {history.map((a) => {
              ...
              return (
                <tr key={a.id} className="border-t border-white/[0.06]"
                  style={{ boxShadow: `inset 3px 0 0 0 ${isApprove ? '#86efac' : '#EF4444'}` }}>
                  ...
                  <Td>
                    <span className="text-[10px] font-bold uppercase tracking-wider rounded px-2 py-0.5"
                      style={{ background: (isApprove ? '#86efac' : '#EF4444') + '22', color: isApprove ? '#86efac' : '#EF4444' }}>
      ...
      <Modal
        ...
        footer={<>
          ...
          <button className="liq-btn" style={{ background: '#86efac', color: '#0B0F1A' }} onClick={doApprove}>Confirm Approve</button>
        </>}
      >
        {confirm && <p className="text-sm text-muted-foreground">Mark <span className="font-mono text-[#3b82f6]">{confirm.block.id}</span> as Completed?</p>}
      </Modal>
          ...
          <div className="liq-card divide-y divide-white/5">
            {recent.map((b) => (
              <div key={b.id} className="p-3 flex items-center gap-3">
                ...
                <span className="font-mono text-[12px] text-[#3b82f6]">{b.id}</span>
